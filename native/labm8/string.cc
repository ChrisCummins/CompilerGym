...
bool EndsWith(const string& full_string, const string& suffix) {
  if (full_string.length() >= suffix.length()) {
    return full_string.compare(full_string.length() - suffix.length(), suffix.length(), suffix) ==
           0;
  } else {
    return false;
  }
...
string ToCamelCase(const string& full_string) {
  ...
  for (auto& s : split_on_whitespace) space_separated_components.push_back(string(s));
