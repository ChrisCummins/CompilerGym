...
def filter_passes(config, pass_iterator: Iterable[Pass]) -> Iterable[Pass]:
    ...
    for pass_ in pass_iterator:
        total_count += 1
        if config.include_pass(pass_._asdict()):
            selected_count += 1
            logging.debug(
    ...
    logging.info(
        ...
        selected_count,
        total_count,
        selected_count / max(total_count, 1) * 100,
    )
...
def main(argv):
    ...
    filtered_passes = filter_passes(load_config(), passes)
